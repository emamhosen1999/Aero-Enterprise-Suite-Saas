...
/**
 * Read composer.json from a host app (saas-host or standalone-host)
 */
export async function readHostAppComposer(hostAppPath: string): Promise<any> {
  const repoRoot = await getRepoRoot();
  const composerPath = path.join(repoRoot, hostAppPath, "composer.json");
...
/**
 * Get packages required by a host app from its composer.json
 */
export async function getHostAppPackages(hostAppPath: string): Promise<string[]> {
  const composer = await readHostAppComposer(hostAppPath);
  if (!composer?.require) {
    return [];
  }
  
  // Extract aero/* packages and convert to aero-* format

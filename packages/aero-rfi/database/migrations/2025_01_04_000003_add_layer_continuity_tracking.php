...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('daily_works', function (Blueprint $table) {
            // Layer Information
            $table->string('layer', 50)->nullable()->after('work_type')->comment('Layer code: sub_base, base_course, etc.');
            $table->integer('layer_order')->nullable()->after('layer')->comment('Layer hierarchy level (1-7)');
            ...
            $table->foreignId('continuity_overridden_by')->nullable()->constrained('users')->onDelete('set null');
            $table->timestamp('continuity_overridden_at')->nullable();
            $table->text('continuity_override_reason')->nullable();
            
            // Indexes
            $table->index(['project_id', 'layer', 'status'], 'idx_layer_status');
            $table->index(['continuity_status', 'can_approve'], 'idx_continuity');
            $table->index(['layer_order', 'start_chainage', 'chainage'], 'idx_layer_chainage');
        });

        // Add comment
        DB::statement("COMMENT ON COLUMN daily_works.continuity_validation_result IS 'Layer sequence validation: {can_approve, gaps, coverage, violations}'");
    }

    public function down(): void
    {
        Schema::table('daily_works', function (Blueprint $table) {
            $table->dropIndex('idx_layer_status');
            $table->dropIndex('idx_continuity');
            $table->dropIndex('idx_layer_chainage');
            $table->dropColumn([
                'layer',
                'layer_order',

...
/**
 * Quality Dashboard - Dynamic Widgets Only
 * 
 * This dashboard renders ONLY dynamic widgets registered by packages.
 * NO hardcoded widgets or container Card wrappers.
 * All quality-specific widgets (inspections, NCRs, metrics) should be
 * registered via the widget system in the aero-quality package.
 */
const QualityDashboard = ({ title = 'Quality Dashboard', dynamicWidgets = [] }) => {
    ...
    return (
        <>
            <Head title={title} />
            <div className="flex flex-col w-full h-full p-4" role="main" aria-label="Quality Dashboard">
                    ...
                    {!hasAnyWidgets && (
                        ...
                        >
                            <div className="p-4 rounded-full bg-warning/10 mb-4">
                                <BeakerIcon className="w-12 h-12 text-warning" />
                            </div>
                            <h3 className="text-xl font-semibold text-foreground mb-2">
                                Quality Dashboard
                            </h3>
                            <p className="text-default-500 max-w-md">
                                Quality widgets will appear here once configured.
                                Contact your administrator to set up inspection, NCR, and metrics widgets.
                            </p>
                        </motion.div>
                    )}
                </div>
...
QualityDashboard.layout = (page) => <App>{page}</App>;

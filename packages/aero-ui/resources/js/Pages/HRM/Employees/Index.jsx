...
const EmployeesList = ({ title, departments, designations, attendanceTypes }) => {
  ...
  const fetchEmployees = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(route('hrm.employees.paginate'), {
        params: {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const { data } = await axios.get(route('hrm.employees.stats'));
      if (data.stats) {
        setStats(data.stats);
      }
    } catch (error) {
  ...
  const EmployeeCard = ({ user, index }) => {
    ...
    return (
      <Card 
        ...
        onPress={() => safeNavigate('hrm.profile', { user: user.id })}
      >
          ...
          <div className="flex items-start gap-3 mb-3 pb-3 border-b border-white/10">
            ...
            <div className="flex-1 min-w-0 flex justify-end">
              <Button
                ...
                onPress={(e) => {
                  e.stopPropagation();
                  safeNavigate('hrm.profile', { user: user.id });
                }}
              >

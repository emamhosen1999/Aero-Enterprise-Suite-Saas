...
const ExpenseClaimsIndex = ({ title }) => {
    ...
    // Permission checks
    const canCreate = auth.permissions?.includes('hrm.expenses.create') || false;
    ...
    const fetchClaims = useCallback(async () => {
        ...
        try {
            ...
            showToast.promise(Promise.reject(error), { error: 'Failed to fetch expense claims' });
        } finally {
            setLoading(false);
        }
    }, [filters, pagination]);
    ...
    return (
        <>
            ...
            <div className="flex flex-col w-full h-full p-4" role="main" aria-label="Expense Claims Management">
                            ...
                            <Card className="transition-all duration-200" style={getThemedCardStyle()}>
                                    ...
                                    <div className={`${!isMobile ? 'p-6' : 'p-4'} w-full`}>
                                        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                                            <div className="flex items-center gap-3 lg:gap-4">
                                                ...
                                                <div>
                                                    <h4 className={`${!isMobile ? 'text-2xl' : 'text-xl'} font-bold`}>Expense Claims</h4>
                                                    <p className={`${!isMobile ? 'text-sm' : 'text-xs'} text-default-500`}>Manage employee expense reimbursement claims</p>
                                                </div>
                                            </div>
                                            {canCreate && (
                                ...
                                <CardBody className="p-6">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-4 mb-6">
                                        <Input label="Search" placeholder="Search claims..." value={filters.search} onChange={(e) => setFilters(prev => ({ ...prev, search: e.target.value }))}
                                    ...
                                    <div className="text-center py-8 text-default-500">
                                        {loading ? "Loading claims..." : "Expense claims table will be displayed here"}
                                    </div>
                                </CardBody>
                            </Card>
                        </motion.div>

...
const RfiFilesModal = ({
    ...
    const fetchFiles = async () => {
        ...
        setLoading(true);
        try {
            const response = await axios.get(route('rfi.daily-works.files.index', dailyWork.id));
        ...
        const promise = new Promise(async (resolve, reject) => {
            try {
                const response = await axios.post(
                    route('rfi.daily-works.files.upload', dailyWork.id),
                    formData,
                    {
                        headers: {
    ...
    const handleDelete = async (mediaId) => {
        ...
        const promise = new Promise(async (resolve, reject) => {
            try {
                await axios.delete(route('rfi.daily-works.files.delete', [dailyWork.id, mediaId]));
    ...
    const handleDownload = (file) => {
        window.open(route('rfi.daily-works.files.download', [dailyWork.id, file.id]), '_blank');
    };

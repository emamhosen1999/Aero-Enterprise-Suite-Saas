...
class RoleController extends Controller
{
                ...
                try {
                    ...
                    $roles = Role::with(['permissions'])
                        ->when(! $isSuperAdmin, function ($query) {
                            ...
                            return $query->whereNotIn('name', ['Super Administrator', 'Super Administrator']);
                        })
                        ->get();
                    ...
                    Log::info('Role management data loaded successfully', [
                        ...
                        'context' => $isPlatform ? 'platform' : 'tenant',
                    ]);
                ...
                try {
                    ...
                    if (! $isSuperAdmin) {
                        $roles = $roles->filter(function ($role) {
                            ...
                            return ! in_array($name, ['Super Administrator', 'Super Administrator']);
                        });
                    }
                    ...
                    // For platform context, we might want LandlordUsers instead
                    $users = collect([]);
                    if (! $isPlatform) {
    ...
    public function storeRole(Request $request)
    {
        ...
        try {
            // Only Super Administrator can create roles for security
            $user = $this->getCurrentUser();
            $isSuperAdmin = $this->isSuperAdmin();
            ...
            // Auto-assign guard based on context:
            // - Platform/Admin context uses 'landlord' guard
            // - Tenant context uses 'web' guard
    ...
    public function updateRole(Request $request, $id)
    {
        ...
        try {
            ...
            // CRITICAL: Super Administrator role cannot be edited (Protected System Role)
            if ($this->isProtectedRole($role)) {
                return response()->json([
                    'error' => 'Super Administrator role cannot be edited. This is a protected system role.',
                ], 403);
            }
            ...
            if (! $this->canManageRole($this->getCurrentUser(), $role)) {
                return response()->json([
    ...
    public function deleteRole($id)
    {
        ...
        try {
            ...
            // CRITICAL: Super Administrator role cannot be deleted (Protected System Role)
            if ($this->isProtectedRole($role)) {
                return response()->json([
                    'error' => 'Super Administrator role cannot be deleted. This is a protected system role.',
                ], 403);
            }
    ...
    public function togglePermission(Request $request)
    {
        ...
        // Format 1: permission_id + value (boolean)
        // Format 2: permission (name) + action ('grant'/'revoke')
        $hasPermissionId = $request->has('permission_id');
        $hasPermissionName = $request->has('permission');
        ...
        try {
            ...
            $role = Role::where('id', $request->role_id)->where('guard_name', $guardName)->first();
            ...
            if ($hasPermissionId) {
                $permission = Permission::where('id', $request->permission_id)
                    ...
                    ->first();
                $shouldGrant = (bool) $request->value;
            } else {
                $permission = Permission::where('name', $request->permission)
                    ->where('guard_name', $guardName)
                    ->first();
                $shouldGrant = $request->action === 'grant';
            }

            if (! $permission) {
                return response()->json([
                    'success' => false,
                    'error' => 'Permission not found for guard: '.$guardName,
                ], 404);
            }
            ...
            // Get all role_has_permissions for the current guard
    ...
    /**
     * Check if a role is protected and cannot be edited or deleted
     */
    private function isProtectedRole(Role $role): bool
    {
        $protectedRoles = ['Super Administrator'];

        return in_array($role->name, $protectedRoles) || ($role->is_protected ?? false);
    }
    ...
    {
        // Protected roles cannot be managed (edited/deleted)
        if ($this->isProtectedRole($role)) {
            return false;
        }

        // Users with roles.update permission can manage non-protected roles
        if ($this->isPlatformContext()) {
            return $user?->isSuperAdmin() ?? false;
        }

        return $user?->can('roles.update') ?? false;
    }

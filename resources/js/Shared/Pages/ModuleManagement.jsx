                                            ...
                                            {selectedRoleId && roles.find(r => r.id === selectedRoleId)?.is_protected ? (
                                                ...
                                                >
                                                    {roles.map(role => (
                                                        <SelectItem 
                                                            key={String(role.id)} 
                                                            value={String(role.id)}
                                                            description={role.is_protected ? 'Full access (protected)' : `Guard: ${role.guard_name}`}
                                                        >
                                                            {role.name}
                                                        </SelectItem>
                                                    ))}
                                                ...
                                                {selectedRoleId && (
                                                    <Button
                                                        ...
                                                        onPress={saveRoleAccess}
                                                        isLoading={roleAccessSaving}
                                                        isDisabled={roles.find(r => r.id === selectedRoleId)?.is_protected}
                                                        style={{
                                                            background: `linear-gradient(135deg, var(--theme-primary), color-mix(in srgb, var(--theme-primary) 80%, var(--theme-secondary)))`,
                                                            borderRadius: getThemeRadius(),
                                                        }}
                                        ...
                                        <CardBody className="p-4">
                                            {!selectedRoleId ? (
                                                <div className="text-center py-8 text-default-500">
                                                    ...
                                                    <p>Select a role above to manage its module access</p>
                                                    <p className="text-sm mt-2">Check modules, sub-modules, components, or actions to grant access</p>
                                                </div>
                                            ) : roles.find(r => r.id === selectedRoleId)?.is_protected ? (
                                                <div className="text-center py-8 text-success">
                                                    <CheckCircleIcon className="w-12 h-12 mx-auto mb-4" />
                                                    <p className="font-semibold">This role has full system access</p>
                                                    <p className="text-sm mt-2 text-default-500">Protected roles automatically have access to all modules</p>
                                                ...
                                                <div className="text-sm text-default-500 flex items-center gap-2">
                                                    <Chip size="sm" color="primary" variant="flat">Tip</Chip>
                                                    <span>Check a parent item (module/sub-module) to grant access to all its children. Uncheck individual children to revoke specific access.</span>
                                                </div>
                                            )}
                                        </CardBody>
                                    </Card>
